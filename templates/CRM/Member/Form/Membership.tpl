...
<input type="hidden" name="dd_payment_instrument" id="dd_payment_instrument" value={$dd_payment_instrument}>
{if $cancelAutoRenew}
        ...
        <tr class="crm-membership-form-block-record_contribution"><td colspan="2">    
          <fieldset id="recordContribution"><legend>{ts}Membership Payment and Receipt{/ts}</legend>
              <table>
                  ...
                  <tr id="mandate-success" class="hiddenElement">
                  <td></td>
                  <td><span class="success-status">{ts}New mandate has been created.{/ts}</span></td>
                  </tr>
                  <tr id="mandateId" class="crm-membership-form-block-mandate_id" style="display:none;">
                      <td class="label">{$form.mandate_id.label}</td>
                      <td>{$form.mandate_id.html}
                      <div id="mandate-dialog" class="hiddenElement"></div>
                      <input type="hidden" name="contactId" id="contactId" value="{$cid}">
                      {if $no_mandate_for_contact eq 1}
                      <div>No Mandate Available for this contact. Please <a href="#" onClick="newMandate(4 , 1);">click here</a> to add one.</div>
                      {/if}
                      </td>
                  </tr>
                  
                  <tr id="newMandateBlock" class="crm-membership-form-block-new-mandate">
                      <td colspan="2">
                        <fieldset><legend>New Mandate</legend>
                            <div class="crm-section first_name-section">
                  			...
                  			<div class="crm-section first_name-section">
                                <div class="label">{$form.sort_code.label}</div>
                  			    <div class="content">{$form.sort_code.html}</div>
                  			    <div class="clear"></div>
                  			</div>
                  		</fieldset>	
                      </td>
...
function newMandate( gid , blockNo ) {
    blockNo = 1;
    var cid = cj('#contactId').val();
    if (cid == '') { 
        //var cid = cj('#contact_' + blockNo ).val();
        var contactHiddenElementForMandate = 'input[name="contact_select_id[' + blockNo +']"]';
        var cid = cj( contactHiddenElementForMandate ).val();
    }
    
    var dataURL = {/literal}"{crmURL p='civicrm/contact/view/cd/edit' q="action=update" h=0 }"{literal};
    dataURL = dataURL + '&groupId=' + gid;
    dataURL = dataURL + '&tableId=' + cid;
    dataURL = dataURL + '&cid=' + cid;
    dataURL = dataURL + '&reset=1';
    dataURL = dataURL + '&createCallback=1';
    
    cj.ajax({
       url: dataURL,
       success: function( content ) {
           cj( '#mandate-dialog').html( content ).dialog({
            ...
            //cj('div').hide();
            //cj('#Direct_Debit_Mandate').focus();
               close: function(event, ui) {
                   cj('#mandate-success-' + blockNo).fadeOut(5000);
      		        //cj('#profiles-' + blockNo).val('');
               }
           });
           //cj("#mandate-dialog").html().;
           var ddMandateHtml = cj("#mandate-dialog").find('#content').html();
           //alert(ddMandateHtml);
           cj("#mandate-dialog").html(ddMandateHtml);
           cj("#mandate-dialog").find('#civicrm-footer').hide();
           //cj("#mandate-dialog").children('div').hide();
           //cj("#mandate-dialog").children('#Direct_Debit_Mandate').show();
       }
    });
}
...
cj( function( ) {
    ...
    cj('#payment_instrument_id').change( function( ) {
        if ( cj(this).val() == cj('#dd_payment_instrument').val()  ) {
            var cssObj = {
            'display' : 'table-row'
            }
        } else {
...
<script type="text/javascript">
cj('#newMandateBlock').hide( );
//cj('#newMandateBlock').hide( );
cj('#mandate_id').change(function() {
  if (cj('#mandate_id').val() == 'new' ) {
    cj('#newMandateBlock').show( );  
  } else {
    cj('#newMandateBlock').hide( );
  }
});
