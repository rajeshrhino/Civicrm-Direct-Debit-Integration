		...
		<td id="{$cd_edit.name}_{$index}" class="section-shown form-item">
			...
			{foreach from=$cd_edit.fields item=element key=field_id}
				...
				<tr>
					...
					{else}
					    <td class="html-adjust">
					    {if $customGroupId eq $DDDetailsGroupId AND $DDDetailsActivityFieldId eq $field_id}
                  <a href="{crmURL p="civicrm/contact/view/activity" q="action=view&id=`$element.field_value`&cid=`$contactId`&reset=1"}">{$element.field_value}</a>
              {else}
                  {$element.field_value}    
               {/if}   
              </td>
