...
<div id="help">
	...
	<tr class="crm-contribution-contributionpage-amount-form-block-currency"><th scope="row" class="label" width="20%">{$form.currency.label}</th>
        ...
        <tr class='crm-contribution-contributionpage-amount-form-block-is_monetary'><th scope='row' class='label' width='20%'>{$form.display_dd_block.label}</th> 
            <td>{$form.display_dd_block.html}
            <input type="hidden" name="display_dd_block_value" id="display_dd_block_value" value={$display_dd_block_value}>
            <input type="hidden" name="display_dd_is_recur" id="display_dd_is_recur" value={$display_dd_is_recur}>
            <br />
              <span class='description'>Check this box if you want to display the online direct debit signup form in the contribution page. Do NOT use this option, if you are using a payment processor.</span></td>
        </tr>
...
<script type="text/javascript">
   ...
   cj( document ).ready( function( ) { 
       ...
       //alert(cj( '#display_dd_block_value' ).val( ));   
       if ( cj( '#display_dd_block_value' ).val( ) == 1 ) {
          if (cj( '#display_dd_is_recur' ).val( ) == 1) {
            cj( '#is_recur' ).attr( 'checked' , true );
            showRecurInterval();
            cj( '#recurFields' ).show( );
          }
       }      
       showRecurringforDirectDebit(  );
    ...
    function showRecurringforDirectDebit(  ) {
        var directddebitId = cj( '#display_dd_block' ).attr( 'checked' );
        var directddebitValue = cj( '#display_dd_block' ).val( );
        if ( directddebitId == false ) {
            if ( cj( '#is_recur' ).attr( 'checked' ) ) {
                cj( '#is_recur' ).removeAttr("checked");
                cj( '#recurFields' ).hide( );
            }
