            ...
            {if $showMandateDropdown}
                {if $no_mandate_for_contact eq 1}
                    <tr id="mandateId" class="crm-membership-form-block-mandate_id" style="display:none;">
                        ...
                        <td>No Mandate Available for this contact. Please <a href="{crmURL p="civicrm/contact/view/cd/edit" q="tableId=$cid&cid=$cid&groupId=$groupId&action=update&reset=1"}">click here</a> to add one.</td>
                    </tr>
                {else}
                    <tr id="mandateId" class="crm-membership-form-block-mandate_id" style="display:none;">
                        <td class="label">{$form.mandate_id.label}</td>
                        <td>{$form.mandate_id.html}</td>
                    </tr>
                {/if}        
            {/if}
...
cj(document).ready( function() {
    ...
    {/literal}
    {if $makeMandateDropdownReadOnly}
        {literal}
            cj('#mandate_id').attr('readonly', true );    
        {/literal}    
    {/if}
    {literal}
