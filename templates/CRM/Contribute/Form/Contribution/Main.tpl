...
{/if}
    ...
    <div id="payment_notice">
      <fieldset class="crm-group payment_notice-group">
          <legend>{ts}SignUp For Direct Debit{/ts}</legend>
          <div class="help">
              <table><tr>
                <td><h2>{ts}Instruction to your Bank or Building Society to pay by Direct Debit{/ts}</h2><br />
                <div class="label">{ts}Banks and Building Societies may not accept Direct Debit Instructions for some types of account{/ts}</div>
                </td>
                <td align="right"><img src="{$userFrameworkBaseURL}/sites/all/modules/civicrm_direct_debit/templates/images/Direct_Debit_Logo.jpg"></td>
              </tr></table> 
          </div>
          <div class="crm-section first_name-section">
              <div class="label">{$form.bank_name.label}</div>
    			    <div class="content">{$form.bank_name.html}</div>
    			    <div class="clear"></div>
    			</div>    
    			<div class="crm-section first_name-section">
              <div class="label">{$form.bank_address.label}</div>
    			    <div class="content">{$form.bank_address.html}</div>
    			    <div class="clear"></div>
    			</div>
    			<div class="crm-section first_name-section">
              <div class="label">{$form.bank_city.label}</div>
    			    <div class="content">{$form.bank_city.html}</div>
    			    <div class="clear"></div>
    			</div>
    			<div class="crm-section first_name-section">
              <div class="label">{$form.bank_county.label}</div>
    			    <div class="content">{$form.bank_county.html}</div>
    			    <div class="clear"></div>
    			</div>
    			<div class="crm-section first_name-section">
              <div class="label">{$form.bank_postcode.label}</div>
    			    <div class="content">{$form.bank_postcode.html}</div>
    			    <div class="clear"></div>
    			</div>
    			<div class="crm-section first_name-section">
              <div class="label">{$form.account_name.label}</div>
    			    <div class="content">{$form.account_name.html}</div>
    			    <div class="clear"></div>
    			</div>
    			<div class="crm-section first_name-section">
              <div class="label">{$form.account_number.label}</div>
    			    <div class="content">{$form.account_number.html}
    			       <div class="description">{ts}{/ts}</div>
    			    </div>
    			    <div class="clear"></div>
    			</div>
    			<div class="crm-section first_name-section">
              <div class="label">
              {if $SortCodeError neq null} <span class="crm-error crm-error-label"> Branch sort code <span class="crm-marker" title="This field is required.">*</span></div> </span>
              {else} Branch sort code <span class="crm-marker" title="This field is required.">*</span></div>
              {/if} 
              
    			    <div class="content">
                  <input maxlength="2" size="2" name="sort_code_part_1" type="text" id="sort_code_part_1" {$SortCodeErrorClass} value="{$smarty.post.sort_code_part_1}" /> - 
                  <input maxlength="2" size="2" name="sort_code_part_2" type="text" ide="sort_code_part_2" {$SortCodeErrorClass} value="{$smarty.post.sort_code_part_2}" /> - 
                  <input maxlength="2" size="2" name="sort_code_part_3" type="text" id="sort_code_part_3" {$SortCodeErrorClass} value="{$smarty.post.sort_code_part_3}" />
                  {if $SortCodeError neq null}<span class="crm-error">{$SortCodeError}</span>{/if} 
              </div>
    			    <div class="clear"></div>
    			</div>
    			<!--<div class="crm-section first_name-section">
              <div class="label">{$form.dd_start_date.label}</div>
    			    <div class="content">
                  {include file="CRM/common/jcalendar.tpl" elementName=dd_start_date}
                  <div class="description">{ts}This is not part of the Instruction to your Bank or Building Society.{/ts}</div>
              </div>
    			    <div class="clear"></div>
    			</div>-->
    			<div class="crm-section first_name-section">
              <div class="label">{$form.collection_day.label}</div>
    			    <div class="content">{$form.collection_day.html}
                  <div class="description">{ts}Payments will be collected on the 1st, 5th, 10th, 15th, 20th or 25th of each month. Please indicate which date you require. If none is selected, payments will be requested on the 20th.{/ts}</div>
              </div>
    			    <div class="clear"></div>
    			</div>
          <div class="help">{ts}<b>The Direct Debit Guarantee</b>
          <ul> 
            <li>• This Guarantee is offered by all banks and building societies that accept instructions to pay Direct Debits</li>
            <li>• If there are any changes to the amount, date or frequency of your Direct Debit <b>[Your Company Name]</b> will notify you 14 working days in advance of  your account being debited or as otherwise agreed. If you request <b>[Your Company Name]</b> to collect a payment, confirmation of the amount and date will be given to you at the time of the request</li>
            <li>• If an error is made in the payment of your Direct Debit by <b>[Your Company Name]</b> or your bank or building society you are entitled to a full and immediate refund of the amount paid from your bank or building society
            <br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;- If you receive a refund you are not entitled to, you must pay it back when <b>[Your Company Name]</b> asks you to</li>
